...
function scr_bg_manager_apply_profile(_inst)
{
    ...
    with (_inst)
    {
        // Ensure level context is correct for THIS room
        if (script_exists(scr_level_prepare_for_room)) {
            scr_level_prepare_for_room(room);
        } else {
            // Fallback: at least ensure sections exist
            scr_level_master_sections_init();
            scr_chunk_build_section_sequences();
        }

        // Use generic master sections (level-agnostic)
        sections = (variable_global_exists("master_sections") && is_array(global.master_sections))
