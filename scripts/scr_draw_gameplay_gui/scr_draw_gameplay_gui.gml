    ...
    for (var i = 0; i < chart_len; i++)
    {
        ...
        if (nref.type == "hold")
        {
            ...
            if (spr != -1) {
                ...
                draw_sprite_ext(spr, subimg_end, end_gx, start_gy, 1, 1, 0, note_col, note_alpha * global.hold_end_alpha);
            }
            else {
        ...
        if (spr != -1) {
            var note_alpha = (variable_struct_exists(nref, "hit") && nref.hit) ? 0.5 : 1.0;
            draw_sprite_ext(spr, subimg_start, start_gx, start_gy, 1, 1, 0, note_col, note_alpha);
        }
        else {
            draw_set_color(note_col);
