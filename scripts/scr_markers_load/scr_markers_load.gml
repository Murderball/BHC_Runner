    ...
    for (var si = 0; si < array_length(save_candidates); si++)
    {
        ...
        if (file_exists(save_name))
        {
            ...
            if (json_save != "")
            {
                ...
                if (is_array(data_save))
                {
                    loaded_save = data_save;
                }
                else if (is_struct(data_save))
                {
                    if (variable_struct_exists(data_save, "markers") && is_array(data_save.markers))
                    {
    ...
    for (var i = 0; i < array_length(defaults); i++)
    {
        ...
        if (file_exists(def_name))
        {
            ...
            if (json_def != "")
            {
                ...
                {
                    if (variable_struct_exists(data_def, "markers") && is_array(data_def.markers))
                    {
