...
function scr_markers_load()
{
    ...
    var save_candidates = [fname_save, "markers_save.json"];
    ...
    for (var si = 0; si < array_length(save_candidates); si++)
    {
        ...
        {
            var json_save = "";
            if (script_exists(scr_load_text_file)) json_save = string_trim(scr_load_text_file(save_name));
            else {
            ...
            if (json_save != "")
            {
                ...
                else if (is_struct(data_save) && variable_struct_exists(data_save, "markers") && is_array(data_save.markers))
    ...
    for (var i = 0; i < array_length(defaults); i++)
    {
        ...
        {
            var json_def = "";
            if (script_exists(scr_load_text_file)) json_def = string_trim(scr_load_text_file(def_name));
            else {
            ...
            if (json_def != "")
            {
                ...
                else if (is_struct(data_def) && variable_struct_exists(data_def, "markers") && is_array(data_def.markers))
