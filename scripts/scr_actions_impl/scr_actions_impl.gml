...
function scr_action_spawn_projectile(_act_name, _judge, _base_dmg, _spd, _life, _pierce, _hit_radius, _proj_col)
{
    var _dmg = _base_dmg;
...
function scr_act_exec_atk1(p, act, def)
{
    ...
    scr_action_spawn_projectile("atk1", judge, 1, 900, 1.2, false, 0, c_aqua);
    return true;
}
...
function scr_act_exec_atk2(p, act, def)
{
    ...
    if (variable_instance_exists(p, "lock_anim")) p.lock_anim("attack", ceil(game_get_speed(gamespeed_fps) * 0.15));
    scr_action_spawn_projectile("atk2", judge, 2, 1050, 1.2, true, 0, col);
    return true;
}
...
function scr_act_exec_atk3(p, act, def)
{
    ...
    if (variable_instance_exists(p, "lock_anim")) p.lock_anim("attack", ceil(game_get_speed(gamespeed_fps) * 0.15));
    scr_action_spawn_projectile("atk3", judge, 3, 850, 1.4, false, 22, col);
    return true;
}
