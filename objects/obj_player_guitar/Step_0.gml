/// obj_player_guitar : Step
/// Jump/Duck/Physics + Guitar projectile attacks as placeholder (ATK1/2/3) + ULT
/// Duck/slide anim plays once and holds last frame
/// Forces stable collision mask to prevent falling through while ducking
...
// ----------------------------------------------------
// Ensure stable collision mask (do NOT let duck sprite change collision)
// ----------------------------------------------------
if (!is_undefined(SPR_IDLE) && SPR_IDLE != 0) {
    if (mask_index != SPR_IDLE) mask_index = SPR_IDLE;
} else {
    // fallback just in case
    if (mask_index != spr_guitarist_idle) mask_index = spr_guitarist_idle;
}
...
// Hook point: preserve existing input/note-hit globals, route through scr_action_try.
// ----------------------------------------------------
if (!global.editor_on)
{
...
// ----------------------------------------------------
// Apply sprite every frame
// ----------------------------------------------------
if (is_undefined(SPR_IDLE) || SPR_IDLE == 0) SPR_IDLE = spr_guitarist_idle;
if (is_undefined(SPR_RUN)  || SPR_RUN  == 0) SPR_RUN  = spr_guitarist_run;
if (is_undefined(SPR_JUMP) || SPR_JUMP == 0) SPR_JUMP = spr_guitarist_jump;
if (is_undefined(SPR_DUCK) || SPR_DUCK == 0) SPR_DUCK = spr_guitarist_duck;
...
// ----------------------------------------------------
// NON-LOOP DUCK: play once, then hold last frame
// ----------------------------------------------------
if (state == "duck")
{
