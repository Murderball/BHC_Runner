...
var t = global.editor_on
    ? scr_chart_time()
    : (scr_song_time() + (global.HITLINE_X / max(pps, 0.0001)) + global.HITLINE_TIME_OFFSET_S);
...
var p01 = (m.t1 > m.t0) ? ((t - m.t0) / max((m.t1 - m.t0), 0.0001)) : 0;
p01 = clamp(p01, 0, 1);
...
if (fade < 1.0)
{
    fade += (delta_time / 1000000.0) / max(fade_s, 0.0001);
